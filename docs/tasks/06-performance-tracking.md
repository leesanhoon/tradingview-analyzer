...
## Việc cần làm
- [x] Thiết kế cách tính win-rate, R:R thực tế, drawdown từ dữ liệu `open_positions` đã đóng.
- [x] Viết hàm tổng hợp theo cặp tiền (group by symbol) và tổng hợp toàn portfolio.
- [x] Tạo runner mới (vd `src/charts/performance-report-runner.ts`) chạy theo lịch GitHub Actions (tuần/tháng).
- [x] Format báo cáo gửi Telegram (win-rate %, R:R trung bình, max drawdown, theo từng cặp).
- [x] Thêm workflow GitHub Actions schedule mới cho báo cáo định kỳ (cron weekly/monthly).
- [ ] (Tuỳ chọn) Thêm lệnh Telegram để gọi báo cáo theo yêu cầu, liên kết với [Phase 09 - /stats](09-stats-command.md).

## Acceptance criteria
- [x] Chạy thử runner cho dữ liệu mẫu ra đúng số liệu win-rate/R:R/drawdown kỳ vọng.
- [x] Báo cáo Telegram gửi đúng định dạng, dễ đọc, đúng lịch.
- [x] Có unit test cho hàm tính toán (không phụ thuộc Telegram/network thật).
