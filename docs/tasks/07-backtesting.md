...
- [ ] Chạy `runBacktest()` trên toàn bộ lịch sử hiện có, ghi lại kết quả thực tế (hit rate so với baseline).
- [ ] Đánh giá xem mô hình dự đoán có thực sự vượt baseline ngẫu nhiên hay không; nếu không, cân nhắc điều chỉnh `lottery-predict.ts`.

### Forex
- [ ] Thiết kế cơ chế backtest: lấy các lệnh đã đóng trong `open_positions`, so sánh quyết định AI tại thời điểm đó với kết quả thực tế.
- [ ] Viết hàm tính tỷ lệ AI dự đoán đúng hướng/đúng entry so với giá thực tế sau đó.

### Betting
- [ ] Thiết kế cơ chế backtest tương tự dùng bảng `matches` — so sánh tín hiệu/kèo AI đề xuất với kết quả trận đấu thực tế.

## Acceptance criteria
- [ ] Có báo cáo backtest lottery chạy được và cho kết quả rõ ràng (hit rate vs baseline).
- [ ] Có cơ chế backtest mới cho Forex và betting, chạy được trên dữ liệu lịch sử thực trong Supabase, output là % độ chính xác.
