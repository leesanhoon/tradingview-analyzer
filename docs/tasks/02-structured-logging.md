...
## Việc cần làm
- [x] Thêm `pino` (+ `pino-pretty` cho dev) vào dependencies.
- [x] Tạo module logger dùng chung (vd `src/shared/logger.ts`) export instance pino cấu hình theo env (pretty ở local, JSON ở CI).
- [x] Thiết kế + tạo migration bảng Supabase `logs` (cột: timestamp, level, message, context/jsonb, source).
- [x] Thêm transport/hook để logger ghi đồng thời vào Supabase `logs` (ít nhất cho level warn/error).
- [x] Thay thế `console.log`/`console.error` trong các file liệt kê ở trên bằng logger mới, theo từng module (chart, betting, lottery).
- [x] Đảm bảo không log secret/API key.

## Acceptance criteria
- [x] Không còn `console.log` trong `src/` (trừ script CLI thuần debug nếu cố ý giữ, cần ghi rõ lý do).
- [x] Lỗi runtime trong các runner chính (chart, betting, lottery) được ghi vào bảng `logs` với đủ context để debug.
- [x] Build/lint không lỗi sau khi thay đổi.
